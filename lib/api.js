...
export async function getAllCategories() {
  const data = await fetchAPI(
    ...
    query GetAllCategories {
      categories(first: 0, where: { parent: null }) {
        nodes {
          name
          slug
        }
...
export async function getSingleCategory(slug) {
  const data = await fetchAPI(
    `
    query {
      posts(first: 20, where: { orderby: { field: DATE, order: DESC }, categoryName: "${slug}" }) {
        edges {
          node {
            title
            excerpt
...
// query ListCategories {
//   categories(first: 1, where: { orderby:NAME, order:DESC }) {
//     nodes {
//       name
//       slug
//       count
